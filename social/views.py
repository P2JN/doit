...
class PostRecommendations(viewsets.GenericAPIView):
    ...
    def get(self, request, user_id, *args, **kwargs):
        ...
        posts = [PostSerializer(post).data for post in Post.objects.filter(id__nin=liked_posts, createdBy__ne=user_id,
                                                                           ...
                                                                           creationDate__gte=datetime.now(zona_horaria) - timedelta(
                                                                               weeks=12))]
        ...
        post_by_followers = PostSerializer(Post.objects.filter(creationDate__gte=datetime.now(zona_horaria) - timedelta(weeks=12),
                                                               id__nin=liked_posts,
