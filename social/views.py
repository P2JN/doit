...
class ParticipateViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        self.perform_create(serializer)
        Stats.objects.filter(createdBy=serializer.instance.createdBy).update_one(
...
class LikePostViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        create_user_notification(instance.post.createdBy,
                                 ...
                                 "Al usuario '" + instance.createdBy.username +
                                 "' le ha gustado tu publicación '" + instance.post.title + "'.",
    ...
    def get(self, request, user_id, *args, **kwargs):
        logged_user_goals = [GoalSerializer(goal).data for goal in
                             Participate.objects.filter(createdBy=user_id).order_by('-creationDate')[0:50].values_list(
                                 'goal')]
        follows = [follow.id for follow in Follow.objects().filter(
            follower=user_id).values_list('follower')]
        ...
        max_followers = sort_by_followers[0].get(
            "numFollowers") if sort_by_followers else 0
        ...
        max_trackings = sort_by_trackings[0].get(
            "numTrackings") if sort_by_trackings else 0
...
class PostRecommendations(viewsets.GenericAPIView):
    ...
    def get(self, request, user_id, *args, **kwargs):
        ...
        liked_posts = [post.id for post in LikePost.objects().filter(
            createdBy=user_id).values_list('post')]
